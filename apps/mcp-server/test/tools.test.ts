...
describe("MCP tool dispatcher", () => {
  it("oww.list_notes returns notes and skipped", async () => {
    ...
    try {
      const result = await dispatchTool("oww_list_notes", {}, makeDeps(vaultRoot));
  ...
  it("oww.list_notes filters by type", async () => {
    const vaultRoot = await createTestVault();
    try {
      const result = await dispatchTool("oww_list_notes", { type: "source" }, makeDeps(vaultRoot));
      const payload = JSON.parse(result.content[0].text);
  ...
  it("oww.read_note returns a validated note", async () => {
    const vaultRoot = await createTestVault();
    try {
      const result = await dispatchTool("oww_read_note", { path: "01 Topics/topic-test.md" }, makeDeps(vaultRoot));
      expect(result.isError).toBeFalsy();
  ...
  it("oww.read_note returns error for non-existent path", async () => {
    const vaultRoot = await createTestVault();
    try {
      const result = await dispatchTool("oww_read_note", { path: "01 Topics/missing.md" }, makeDeps(vaultRoot));
  ...
  it("oww.get_vault_status counts notes by kind", async () => {
    ...
    try {
      await dispatchTool("oww_create_claim_note", {
        title: "Claim one", statement: "Statement.", topicIds: ["topic-test"]
      }, deps);
      await dispatchTool("oww_create_source_note", {
        title: "Source one", topicIds: ["topic-test"]
      }, deps);

      const result = await dispatchTool("oww_get_vault_status", {}, deps);
      expect(result.isError).toBeFalsy();
  ...
  it("oww.get_policy returns the folder policy", async () => {
    const vaultRoot = await createTestVault();
    try {
      const result = await dispatchTool("oww_get_policy", {}, makeDeps(vaultRoot));
      expect(result.isError).toBeFalsy();
  ...
  it("returns error result for unknown tool name", async () => {
    const vaultRoot = await createTestVault();
    try {
      const result = await dispatchTool("oww_nonexistent", {}, makeDeps(vaultRoot));
      expect(result.isError).toBe(true);
  ...
  it("oww.get_invalid_notes returns empty list when all notes are valid", async () => {
    const vaultRoot = await createTestVault();
    try {
      const result = await dispatchTool("oww_get_invalid_notes", {}, makeDeps(vaultRoot));
      expect(result.isError).toBeFalsy();
